...
def graficar_pendientes_tiempo_tratamiento(tratamiento, intervalo_tiempo=1.0, porcentaje=None):
    ...
    plt.title(f"Derivadas de Fluorescencia")
...
def segmentar_y_encontrar_centroides_multiples(imagen, radio_min_px=4, radio_max_px=8, threshold_binarization=16):
    ...
    for p in props:
        ...
        if (r>=radio_min_px and r<=radio_max_px):
            #print(p.centroid,r)
            cv2.circle(imagen_allchannels, (int(x), int(y)), int(r), (255, 0, 0), 1)  # círculo rojo
            cv2.circle(imagen_allchannels, (int(x), int(y)), 2, (255, 0, 0), -1)  # punto en el centro
            centroides_encontrados.append((int(x),int(y)))
...
def main():
    ...
    threshold_graphs=20
    time_choose_image=-1
    folder_name=os.path.join('photos_treatments_06052026_2', '06052026_test_in_OneFile')
    try:
        ...
        try:
            ...
            print('No se pudo abrir el archivo, verifique el nombre de la carpeta')
            return
        #start modification
        ...
        if len(image_tif.shape)==5:
            ...
            axes[0,0].set_title(f'Canal 1 LED apagado- Tiempo Inicial')# {time_idx_start}')
            axes[0,0].axis('off')

            axes[0,1].imshow(image_tif[time_idx_start, 1, :, :, :])
            axes[0,1].set_title(f'Canal 2 LED encendido - Tiempo Inicial')# {time_idx_start}')
            axes[0,1].axis('off')

            axes[1,0].imshow(image_tif[time_idx_end, 0, :, :, :])
            axes[1,0].set_title(f'Canal 1 LED apagado - Tiempo Final')# {time_idx_end}')
            axes[1,0].axis('off')

            axes[1,1].imshow(image_tif[time_idx_end, 1, :, :, :])
            axes[1,1].set_title(f'Canal 2 LED encendido - Tiempo Final')# {time_idx_end}')
            axes[1,1].axis('off')

            plt.tight_layout()
            plt.show()
            data = [image_tif[i, 1, :, :, :] for i in range(image_tif.shape[0])]

        elif len(image_tif.shape)==4:
            # Separar por canales para mayor claridad
            # Estructura: [Tiempo, Y, X, RGB]
            ...
            time_idx_start=1
            time_idx_end=-1
            image_tif=image_tif.transpose(0,2,3,1)
            #print(image_tif.shape)
            fig, axes = plt.subplots(2,1 , figsize=(8, 10))
            time_end=len(image_tif[:,0,0,0])
            axes[0].imshow(image_tif[time_idx_start, :, :, :])
            axes[0].set_title(f'LED encendido - Tiempo Inicial 0')# {time_idx_end}')
            axes[0].axis('off')

            
            axes[1].imshow(image_tif[time_idx_end, :, :, :])
            axes[1].set_title(f'LED encendido - Tiempo Final {time_end}')# {time_idx_end}')
            axes[1].axis('off')

            plt.tight_layout()
            plt.show()

            data = [image_tif[i, :, :, :] for i in range(image_tif.shape[0])]
        ...
        path_metadata = os.path.join(folder_name, '*_metadata.txt')#Search any file that has metadata.txt in the name
    ...
    for celda in celdas:
        ...
        try:
            circulo = obtener_circulos(img_celda, celda.tipo, threshold_transform, plotear=False)
        except Exception as e:
